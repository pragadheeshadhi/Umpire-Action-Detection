import cv2
import mediapipe as mp
import numpy as np
from tensorflow.keras.models import load_model
from collections import deque  # Used for storing last 10 frames

# 📌 Load Model
#model_path = r"C:\PROJECT\Umpire Environment\All umpire signals LSTM+Mediapipe\model\model.h5"

model_path = r"Models1/LSTM_Model2.h5"

model = load_model(model_path, compile=False)
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])

# 📌 Define Class Labels (Modify as per your dataset)
class_labels = ["Out", "Byes", "Four", "Six", "No Ball", "Wide", "Free Hit", "Leg Byes"]  # Update as needed

# 📌 Initialize Mediapipe
mpPose = mp.solutions.pose
pose = mpPose.Pose()
mpHands = mp.solutions.hands
hands = mpHands.Hands()
mpDraw = mp.solutions.drawing_utils

# 📌 Extract Pose Landmarks
def extract_pose_landmarks(results_pose):
    pose_lm = []
    if results_pose.pose_landmarks:
        for lm in results_pose.pose_landmarks.landmark:
            pose_lm.extend([lm.x, lm.y, lm.z, lm.visibility])
    return pose_lm if len(pose_lm) == 132 else pose_lm + [0.0] * (132 - len(pose_lm))

# 📌 Extract Hand Landmarks
def extract_hand_landmarks(results_hands):
    hand_lm = []
    if results_hands.multi_hand_landmarks:
        for hand_landmarks in results_hands.multi_hand_landmarks:
            for lm in hand_landmarks.landmark:
                hand_lm.extend([lm.x, lm.y, lm.z])
    return hand_lm if len(hand_lm) == 195 else hand_lm + [0.0] * (195 - len(hand_lm))

# 📌 Store Last 10 Frames (for LSTM)
frame_buffer = deque(maxlen=10)

# 📌 Start Webcam
cap = cv2.VideoCapture(0)

while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
        break

    # Convert to RGB
    frameRGB = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    results_pose = pose.process(frameRGB)
    results_hands = hands.process(frameRGB)

    # Extract Landmarks
    pose_lm = extract_pose_landmarks(results_pose)
    hand_lm = extract_hand_landmarks(results_hands)

    # Combine Pose + Hands (Total: 327 values)
    frame_landmarks = pose_lm + hand_lm
    frame_buffer.append(frame_landmarks)  # Store in buffer

    # Only predict if we have 10 frames
    if len(frame_buffer) == 10:
        input_data = np.array(frame_buffer).reshape(1, 10, 327)  # Shape: (1, 10, 327)
        
        # Debug: Print Shape
        print("Model Expected Input Shape:", model.input_shape)
        print("Actual Input Shape:", input_data.shape)

        # Make Prediction
        prediction = model.predict(input_data)
        predicted_index = np.argmax(prediction)  # Get index of highest probability
        predicted_label = class_labels[predicted_index]  # Get actual signal name

        # Display Prediction
        cv2.putText(frame, f"Prediction: {predicted_label}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)

    # Show Frame
    cv2.imshow("Live Prediction", frame)

    if cv2.waitKey(1) == ord('q'):
        break

cap.release()
cv2.destroyAllWindows()
