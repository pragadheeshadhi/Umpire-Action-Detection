...
X, y = [], []
no_of_timesteps = 30  # 📌 Defines sequence length
...
# 📌 Build LSTM Model (Matching the Image Architecture)
model = Sequential([
    Input(shape=(30, 327)),  # 📌 Matches (None, 30, 1662)
    ...
    LSTM(64),  # 📌 Third LSTM Layer (returns a single output)
    Dropout(0.3),
...
for i, signal in enumerate(signals[:6]):  # 📌 Only 6 classes as per image
    print(f"🔹 {signal}: {per_class_acc[i]:.4f}")

# 📌 Plot Confusion Matrix
plt.figure(figsize=(8, 8))
sns.heatmap(conf_matrix, annot=True, fmt="d", cmap="Blues", xticklabels=signals[:6], yticklabels=signals[:6])
...
model.save("Models4/LSTM_Model_T9.h5")
